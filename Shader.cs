...
namespace JLGraphics
{
    ...
    public sealed class Shader : IName
    {
        ...
        Dictionary<int, Texture> id2Tex = new Dictionary<int, Texture>();
        Dictionary<Texture, int> tex2unit = new Dictionary<Texture, int>();
        internal void MapPropertyToTexture(int property, Texture texture)
        {
            //0 get new texture unit if we are adding a new property mapped to a new texture
            //1 reuse texture unit if we are adding a new property mapped to an existing texture
            //2 reuse texture unit if we are adding an existing property mapped to a new texture
            //3 reuse texture unit if we are adding an existing property mapped to an existing texture

            var propExists = id2Tex.TryGetValue(property, out var existingTexture);
            var textureExists = tex2unit.TryGetValue(texture, out var existingTextureUnit);
            ...
            {
                case 0:
                    unit = availableTextureSlots.Pop();
                    id2Tex.Add(property, texture);
                    tex2unit.Add(texture, unit);
                    SetInt(property, unit);
                    break;
                case 1:
                    unit = existingTextureUnit;
                    id2Tex.Add(property, texture);
                    SetInt(property, unit);
                    break;
                case 2:
                    unit = tex2unit[existingTexture];
                    tex2unit.Remove(existingTexture);
                    tex2unit.Add(texture, unit);
                    id2Tex[property] = texture;
                    SetInt(property, unit);
                    break;
                case 3:
                    unit = existingTextureUnit;
